...
void read_netlist(INP const char *net_file, INP const t_arch *arch,
	...
	// fill up the logical_block_types vector
	// this vector holds the type of each logical block
	
	int total_types = 0;
	std::unordered_map<std::string, int> type_map;
	for (i = 0; i < num_logical_blocks; i++) {
		std::string type = logical_block[i].pb->pb_graph_node->pb_type->name;
		if (type_map.find(type) == type_map.end()) {
			type_map[type] = total_types;
			total_types++;
		}
	}
	...
	for (i = 0; i < num_logical_blocks; i++) {
		std::string type = logical_block[i].pb->pb_graph_node->pb_type->name;
		std::vector<int> type_vector(total_types, 0);
		type_vector[type_map[type]] = 1;
		//std::cout << "Type of block " << i << " is " << type << std::endl;
		logical_block_types.push_back(type_vector);
	}
	...
	*ext_ncount = 0;
	...
	for (i = 0; i < *ext_ncount; i++) {
		for (j = 1; j <= (*ext_nets)[i].num_sinks; j++) {
			boolean is_global_net =
					static_cast<boolean>((*ext_nets)[i].is_global);
