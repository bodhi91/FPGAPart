...
static enum e_block_pack_status try_place_logical_block_rec(
		...
		INP t_cluster_placement_stats *cluster_placement_stats_ptr,
		INP boolean is_root_of_chain, INP t_pb_graph_pin *chain_root_pin, INOUTP t_lb_router_data *router_data);
...
void do_clustering(const t_arch *arch, t_pack_molecule *molecule_head,
	...
	*****************************************************************/
	...
	std::vector<std::pair<float, std::vector<t_net_pin> > > top_n_paths = get_top_n_timing_paths(slacks, 1000);

	std::string top_n_paths_file = "logical.top_n_paths";
	FILE * top_n_paths_fp = fopen(top_n_paths_file.c_str(), "w");
	for (int i = 0; i < top_n_paths.size(); ++i) {
		fprintf(top_n_paths_fp, "%f", top_n_paths[i].first);
		// add a space to separate the float from the path
		fprintf(top_n_paths_fp, " ");
		for (int j = 0; j < top_n_paths[i].second.size(); ++j) {
			fprintf(top_n_paths_fp, "%d", top_n_paths[i].second[j]);
			if (j != top_n_paths[i].second.size() - 1) {
				fprintf(top_n_paths_fp, " ");
			}
		}
		fprintf(top_n_paths_fp, "\n");
	}
	fclose(top_n_paths_fp);

	print_block_criticalities("logical.block_criticalities");
	logical_block_criticalities.resize(num_logical_blocks);
	for (int iblock = 0; iblock < num_logical_blocks; iblock++) {
		logical_block_criticalities[iblock] = block_criticality[iblock];
	}
	print_criticality(slacks, "logical.slacks");
	logical_net_criticalities.resize(g_atoms_nlist.net.size());
	// fill up logical_net_criticalities
	for (int inet = 0; inet < g_atoms_nlist.net.size(); inet++) {
		logical_net_criticalities[inet] = 0;
		for (int ipin = 1; ipin < g_atoms_nlist.net[inet].pins.size(); ipin++) {
			int iblock = g_atoms_nlist.net[inet].pins[ipin].block;
			if (block_criticality[iblock] > logical_net_criticalities[inet]) {
				logical_net_criticalities[inet] = block_criticality[iblock];
			}
		}
	}
