...
static void update_slacks(t_slack * slacks, int source_clock_domain, int sink_clock_domain, float criticality_denom,
	boolean update_slack, boolean is_final_analysis
#if SLACK_DEFINITION == 'I' 
	, float smallest_slack_in_design
#endif
	);
...
#if SLACK_DEFINITION == 'I'
static float find_least_slack(boolean is_prepacked);
#endif
...
/*  Performs timing analysis on the circuit.  Before this routine is called, t_slack * slacks 
	...
	float smallest_slack_in_design = find_least_slack(is_prepacked);
		...
		for (sink_clock_domain = 0; sink_clock_domain < g_sdc->num_constrained_clocks; sink_clock_domain++) {
			if (g_sdc->domain_constraint[source_clock_domain][sink_clock_domain] > NEGATIVE_EPSILON) { /* i.e. != DO_NOT_ANALYSE */
				...
				update_slacks(slacks, source_clock_domain, sink_clock_domain, criticality_denom, 
					update_slack, is_final_analysis
#if SLACK_DEFINITION == 'I' 
...
std::vector<float> get_net_criticalities(t_slack* slacks) {
	std::vector<t_vnet> & tnets = *timing_nets;
	std::vector<float> criticalities;
	for (int inet = 0; inet < num_timing_nets; inet++) {
		int num_edges = tnets[inet].num_sinks();
		for (int iedge = 0; iedge < num_edges; iedge++) {
			criticalities.push_back(slacks->timing_criticality[inet][iedge + 1]);
		}
	}
	return criticalities;
}

bool lambda_sort_path(const std::pair<float, int> & a, const std::pair<float, int> & b) {
	return a.first > b.first;
}

std::vector<std::pair<float, std::vector<t_net_pin> > > get_top_n_timing_paths(t_slack * slacks, int n) {
	std::vector<t_vnet> & tnets = *timing_nets;
	std::vector<std::pair<float, int> > top_n_critical_nets;
	int num_edges;
	for (int inet = 0; inet < num_timing_nets; inet++) {
		num_edges = tnets[inet].num_sinks();
		for (int iedge = 0; iedge < num_edges; iedge++) {
			top_n_critical_nets.push_back(std::make_pair(slacks->timing_criticality[inet][iedge + 1], inet));
		}
	}

	sort(top_n_critical_nets.begin(), top_n_critical_nets.end(), lambda_sort_path);
	top_n_critical_nets.resize(min(n, (int)top_n_critical_nets.size()));

	std::vector<std::pair<float, std::vector<t_net_pin> > > t_paths;

	for (int i = 0; i < (int)top_n_critical_nets.size(); i++) {
		int inet = top_n_critical_nets[i].second;
		int num_edges = tnets[inet].num_sinks();
		float slack = top_n_critical_nets[i].first;
		std::vector<t_net_pin> path;
		for (int iedge = 0; iedge < num_edges; iedge++) {
			path.push_back(tnets[inet].pins[iedge + 1]);
		}
		path.push_back(tnets[inet].pins[0]);
		t_paths.push_back(std::make_pair(slack, path));
	}
					...
					for (int i = 0; i < num_at_level; i++) {
						...
						if (num_edges == 0) { /* sink */
							...
							if (g_sdc->num_cf_constraints > 0) {
								...
								char *sink_register_name = find_tnode_net_name(inode, is_prepacked);
...
bool tnode_comparator(int a, int b) {
    return tnode[a].T_req - tnode[a].T_arr < tnode[b].T_req - tnode[b].T_arr;
}

std::vector<std::pair<int, float> > get_tnode_slacks() {
	t_linked_int *critical_path_head, *curr_crit_node, *prev_crit_node;
    int inode, iedge, to_node, num_at_level_zero, i, j, crit_node = OPEN, num_edges;
    int source_clock_domain = UNDEFINED, sink_clock_domain = UNDEFINED;
    float min_slack = HUGE_POSITIVE_FLOAT, slack;
	...
	std::vector<std::pair<int, float> > tnode_slacks;
	for (int i = 0; i < num_tnodes; i++) {
		if (has_valid_T_arr(i) && has_valid_T_req(i)) {
			tnode_slacks.push_back(std::make_pair(i, tnode[i].T_req - tnode[i].T_arr));
		}
	}
	return tnode_slacks;
}

std::vector<t_linked_int*> allocate_and_load_top_n_critical_paths(int n) {
    /* Finds the top n critical paths and returns a list of the tnodes on the critical *
     * path in a linked list, from the path SOURCE to the path SINK.            */
    std::vector<t_linked_int*> critical_path_heads;
    t_linked_int *critical_path_head, *curr_crit_node, *prev_crit_node;
    int inode, iedge, to_node, num_at_level_zero, i, j, crit_node = OPEN, num_edges;
    int source_clock_domain = UNDEFINED, sink_clock_domain = UNDEFINED;
    float min_slack = HUGE_POSITIVE_FLOAT, slack;
    ...
    // sort the nodes according to slack
    ...
    std::sort(sorted_nodes.begin(), sorted_nodes.end(), tnode_comparator);
    int num_paths = n > num_tnodes ? num_tnodes : n;
    // find the top n critical paths
    for (int path = 0; path < num_paths; path++) {
        /* Start at the source (level-0) tnode with the least slack (T_req-T_arr).
        This will form the head of our linked list of tnodes on the critical path. */
        crit_node = sorted_nodes[path];
        min_slack = tnode[crit_node].T_req - tnode[crit_node].T_arr;
        critical_path_head = (t_linked_int *) my_malloc(sizeof(t_linked_int));
        critical_path_head->data = crit_node;
        assert(crit_node != OPEN);
        prev_crit_node = critical_path_head;
        ...
        prev_crit_node->next = NULL;
        critical_path_heads.push_back(critical_path_head);
    }
    return critical_path_heads;
}
...
#if SLACK_DEFINITION == 'R' || SLACK_DEFINITION == 'I'
...
void print_mapped_top_n_critical_paths(int n, std::vector<int>& clusters, const char *fname) {
    std::vector<t_linked_int*> top_paths = allocate_and_load_top_n_critical_paths(n);
    t_linked_int *critical_path_head, *critical_path_node;
    FILE *fp;
    int non_global_nets_on_crit_path, global_nets_on_crit_path;
    int tnodes_on_crit_path, inode, iblk, inet;
    ...
    vector<t_vnet> & tnets = *timing_nets;
    fp = my_fopen(fname, "w", 0);
    float clock_period = get_critical_path_delay();
    vpr_printf_info("Clock period %g\n", clock_period);
    for (auto path : top_paths) {
        critical_path_head = path;
        ...
        while (critical_path_node != NULL) {
            ...
            if (type == TN_CB_OPIN) {
                get_tnode_block_and_output_net(inode, &iblk, &inet);
                slack = (tnode[inode].T_req - tnode[inode].T_arr);
                auto block_name = block[iblk].name;
            ...
            if (next_crit_node != NULL) {
                auto downstream_node = next_crit_node->data;
        ...
        vpr_printf_info("Slack %g \n", slack);
        float norm_slack = 1.0 - (slack / clock_period);
        vpr_printf_info("Slack %g Delay %g \n", norm_slack, total_del);
        fprintf(fp, "%g ", slack);
        for (auto blk : blocks) {
            fprintf(fp, "%s ", blk);
        }
...
void print_top_n_critical_paths(int n, const char *fname) {
    std::vector<t_linked_int*> top_paths = allocate_and_load_top_n_critical_paths(n);
    t_linked_int *critical_path_head, *critical_path_node;
    FILE *fp;
    int non_global_nets_on_crit_path, global_nets_on_crit_path;
