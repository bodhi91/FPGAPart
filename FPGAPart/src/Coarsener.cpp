...
HGraphPtr Coarsener::ContractHypergraph(
    const HGraphPtr& hgraph,
    const std::vector<int>& vertex_cluster_id_vec) const
{
  // map current vertex_id to cluster_id
  // the remaining arguments are related to clusters
  int max_cluster_id = *std::max_element(vertex_cluster_id_vec.begin(),
                                         vertex_cluster_id_vec.end());
  Matrix<float> vertex_weights_c(
      max_cluster_id + 1,
      std::vector<float>(hgraph->GetVertexDimensions(), 0.0));
  std::vector<int> community_attr_c(max_cluster_id + 1, -1);
  std::vector<int> fixed_attr_c(max_cluster_id + 1, -1);
  Matrix<float> placement_attr_c(
      max_cluster_id + 1,
      std::vector<float>(hgraph->GetPlacementDimensions(), 0.0));

  for (int i = 0; i < vertex_cluster_id_vec.size(); i++) {
    vertex_weights_c[vertex_cluster_id_vec[i]]
        = hgraph->GetVertexWeights(i)
          + vertex_weights_c[vertex_cluster_id_vec[i]];
    if (hgraph->HasCommunity()) {
      community_attr_c[vertex_cluster_id_vec[i]] = hgraph->GetCommunity(i);
    }
    if (hgraph->HasPlacement()) {
      placement_attr_c[vertex_cluster_id_vec[i]]
          = hgraph->GetPlacement(i)
            + placement_attr_c[vertex_cluster_id_vec[i]];
    }
    if (hgraph->HasFixedVertices()) {
      fixed_attr_c[vertex_cluster_id_vec[i]] = hgraph->GetFixedAttr(i);
    }
  }

  // create the contracted hypergraph based on the vertex matching in
  // vertex_cluster_id_vec
  return Contraction(hgraph,
                     vertex_cluster_id_vec,
                     vertex_weights_c,
                     community_attr_c,
                     fixed_attr_c,
...
// infrastructure for FPGrowth

FPGrowth::FPGrowth(int min_support, int num_threads)
    : root_(std::make_shared<Node>(-1, 0)),
      min_support_(min_support),
      num_threads_(num_threads)
{
}

void FPGrowth::fit(const dataset_t& transactions)
{
  // Step 1: Build frequent itemsets
  item_count_map_t frequent_items = build_frequent_itemsets(transactions);

  // Step 2: Construct the FP-Tree
  construct_fp_tree(transactions, frequent_items);

  // Step 3: Parallel mining
  workers_.clear();
  patterns_.resize(num_threads_);
  for (int i = 0; i < num_threads_; ++i) {
    workers_.emplace_back(
        &FPGrowth::parallel_mine, this, root_, std::vector<item_t>(), i);
  }
...
void FPGrowth::print_frequent_patterns() const
{
  for (const auto& pattern_map : patterns_) {
    for (const auto& pair : pattern_map) {
      const auto& pattern = pair.first;  // Vector of integers
      const auto& count = pair.second;   // Frequency count

      std::cout << "Timing Path: ";
...
item_count_map_t FPGrowth::build_frequent_itemsets(
    const dataset_t& transactions)
{
  item_count_map_t item_counts;
  ...
  for (auto it = item_counts.begin(); it != item_counts.end();) {
    if (it->second < min_support_) {
      it = item_counts.erase(it);
    } else {
      ++it;
    }
  }
...
void FPGrowth::construct_fp_tree(const dataset_t& transactions,
                                 const item_count_map_t& frequent_items)
{
  for (const auto& transaction : transactions) {
    std::vector<item_t> sorted_transaction;
    for (const auto& item : transaction) {
    ...
    std::sort(sorted_transaction.begin(),
              sorted_transaction.end(),
              [&](const item_t& a, const item_t& b) {
                return frequent_items.at(a)
                       > frequent_items.at(b);  // Descending order of frequency
              });

    auto current_node = root_;
    for (const auto& item : sorted_transaction) {
      if (current_node->children.find(item) == current_node->children.end()) {
        current_node->children[item] = std::make_shared<Node>(item, 0);
      }
...
void FPGrowth::parallel_mine(const std::shared_ptr<Node>& node,
                             const std::vector<item_t>& prefix,
                             int thread_index)
{
  std::unordered_map<std::vector<item_t>, int, VectorHash> local_result;
  mine_fp_tree(node, prefix, local_result);

  // Add unique patterns to the global set and thread-specific results
  {
    std::lock_guard<std::mutex> lock(tree_mutex_);
    for (const auto& [pattern, count] : local_result) {
      // Check if pattern is already in the global set
      if (global_pattern_set_.find(pattern) == global_pattern_set_.end()) {
        global_pattern_set_.insert(pattern);  // Add to global set
        patterns_[thread_index][pattern]
            = count;  // Add to thread-specific patterns
      }
    }
  }
}

void FPGrowth::mine_fp_tree(
    ...
    std::unordered_map<std::vector<item_t>, int, VectorHash>& result)
{
  ...
  for (const auto& [item, child] : node->children) {
    std::vector<item_t> new_prefix = prefix;
    new_prefix.push_back(item);

    // Sort the pattern to ensure consistent ordering
    std::sort(new_prefix.begin(), new_prefix.end());

    result[new_prefix] = child->count;

    // Recursively mine the subtree
    mine_fp_tree(child, new_prefix, result);
  }
}

std::vector<std::vector<int>> FPGrowth::cluster(
    std::vector<int> comm_attr) const
{
  std::vector<std::vector<int>> clusters;
  std::unordered_set<int> assigned_vertices;

  // Flatten patterns_ into a single vector of pairs (pattern, frequency)
  std::vector<std::pair<std::vector<int>, int>> flattened_patterns;
  for (const auto& pattern_map : patterns_) {
    for (const auto& pair : pattern_map) {
      flattened_patterns.emplace_back(pair.first, pair.second);
    }
  }

  // Sort flattened patterns by score: frequency * size
  std::sort(flattened_patterns.begin(),
            flattened_patterns.end(),
            [](const auto& a, const auto& b) {
              return (a.second * a.first.size()) > (b.second * b.first.size());
            });

  // Create clusters through greedy assignment
  for (const auto& [pattern, frequency] : flattened_patterns) {
    // Temporary map to group vertices by community attribute
    std::unordered_map<int, std::vector<int>> community_groups;

    for (int vertex : pattern) {
      // Skip vertex if it's already assigned
      if (assigned_vertices.find(vertex) != assigned_vertices.end()) {
        continue;
      }

      // Group by community attribute if comm_attr is provided
      if (!comm_attr.empty()) {
        community_groups[comm_attr[vertex]].push_back(vertex);
      } else {
        // Default to a single group with a dummy key for no community
        // attributes
        community_groups[-1].push_back(vertex);
      }
    }

    // Process community groups into separate clusters
    for (const auto& [community, group] : community_groups) {
      if (group.size() > 1) {  // Only add clusters with more than one vertex
        clusters.push_back(group);
        for (int vertex : group) {
          assigned_vertices.insert(vertex);  // Mark vertices as assigned
        }
      }
    }
  }
