...
// Read the timing file and collect the net slacks and node slacks
void TritonPart::ReadTimingPaths(const std::string& timing_paths_file_name)
{
  // Each path is written as follows:
  // slack ver1 ver2 ... vern

  std::ifstream timing_paths_file(timing_paths_file_name);  // Open the file
  std::string cur_line;
  while (std::getline(timing_paths_file, cur_line)) {
    std::stringstream ss(cur_line);
    TimingPath timing_path;
    ss >> timing_path.slack;
    int vertex_id;
    while (ss >> vertex_id) {
      timing_path.path.push_back(vertex_id);
    }
    if (!timing_path.path.empty()) {
      timing_paths_.push_back(timing_path);
    }
  }

  timing_paths_file.close();

  // Find the hyperedges that each timing path contains

  for (auto& timing_path : timing_paths_) {
    for (const auto& vertex_id : timing_path.path) {
      for (int hyperedge_id = 0; hyperedge_id < num_hyperedges_;
           ++hyperedge_id) {
        if (std::find(hyperedges_[hyperedge_id].begin(),
                      hyperedges_[hyperedge_id].end(),
                      vertex_id)
            != hyperedges_[hyperedge_id].end()) {
  ...
  // for each hyperedge, find the minimum slack of all the paths that contain it

  for (int hyperedge_id = 0; hyperedge_id < num_hyperedges_; ++hyperedge_id) {
    for (const auto& timing_path : timing_paths_) {
      if (std::find(
              timing_path.arcs.begin(), timing_path.arcs.end(), hyperedge_id)
          != timing_path.arcs.end()) {
        hyperedge_slacks_[hyperedge_id]
            = std::min(hyperedge_slacks_[hyperedge_id], timing_path.slack);
      }
    }
  }

  logger_->report("[STATUS] Read {} timing paths from file",
                  timing_paths_.size());
}

// Read the FPGA-based netlist (hypergraph)
void TritonPart::ReadFpgaGraph(const std::string& hypergraph_file,
                               const std::string& community_file,
                               const std::string& group_file,
                               const std::string& timing_file,
                               const std::string& pre_packed_file)
{
  std::ifstream hypergraph_file_input(hypergraph_file);
  if (!hypergraph_file_input.is_open()) {
    logger_->error(
        PAR, 16, "Cannot open the input hypergraph file : {}", hypergraph_file);
  }

  logger_->report("[INFO] Hypergraph file : {}", hypergraph_file);

  if (community_file.empty() == false) {
    logger_->report("[INFO] Community file : {}", community_file);
  }

  if (group_file.empty() == false) {
    logger_->report("[INFO] Group file : {}", group_file);
  }

  if (timing_file.empty() == false) {
    logger_->report("[INFO] Timing file : {}", timing_file);
  }
  ...
  for (int i = 0; i < num_vertices_; i++) {
    if (vertex_weight_flag == true) {
      // the weight is a vector
      // the first entry is the criticality of the block
      // the next entries are the actual weight vector of that block
      // we do not know the vertex dimensions yet untill we have read the first
      // vertex weight vector
      std::getline(hypergraph_file_input, cur_line);
      std::istringstream cur_line_buf(cur_line);
      std::vector<float> vvec{std::istream_iterator<float>(cur_line_buf),
                              std::istream_iterator<float>()};
      if (vertex_weights_.empty()) {
        vertex_dimensions_ = vvec.size() - 1;
      }
      std::vector<float> vwts(vvec.begin() + 1, vvec.end());
      vertex_weights_.push_back(vwts);
      vertex_criticalities_.push_back(vvec[0]);
    } else {
  ...
  vertex_dimensions_ = vertex_weights_[0].size();
  ...
  if (!community_file.empty()) {
    ...
    if (!community_file_input.is_open()) {
      logger_->error(
          PAR, 20, "Cannot open the community file : {}", community_file);
    }
  ...
  if (!group_file.empty()) {
    ...
    if (!group_file_input.is_open()) {
      logger_->error(PAR, 38, "Cannot open the group file : {}", group_file);
    }
    molecule_attr_.clear();
    std::string cur_line;
    while (std::getline(group_file_input, cur_line)) {
      size_t vertex_pos = cur_line.find("Vertex ID:");
      size_t molecule_pos = cur_line.find("Molecule ID:");

      if (vertex_pos != std::string::npos
          && molecule_pos != std::string::npos) {
        // extract vertex ID and molecule ID
        int vertex_id = std::stoi(
            cur_line.substr(vertex_pos + 10, molecule_pos - (vertex_pos + 10)));
        int molecule_id = std::stoi(cur_line.substr(molecule_pos + 12));
        molecule_attr_[vertex_id] = molecule_id;
      }
    }
  ...
  if (!timing_file.empty()) {
    std::ifstream timing_file_input(timing_file);
    if (!timing_file_input.is_open()) {
      logger_->error(PAR, 39, "Can not open the timing file : {}", timing_file);
    }
    ReadTimingPaths(timing_file);
  ...
  if (!pre_packed_file.empty()) {
    std::ifstream pre_packed_file_input(pre_packed_file);
    if (!pre_packed_file_input.is_open()) {
      logger_->error(
          PAR, 40, "Can not open the pre-packed file : {}", pre_packed_file);
    }
    std::string cur_line;
    while (std::getline(pre_packed_file_input, cur_line)) {
      std::istringstream cur_line_buf(cur_line);
      std::vector<int> packed_vertices{std::istream_iterator<int>(cur_line_buf),
                                       std::istream_iterator<int>()};
      pre_packed_vertices_.push_back(packed_vertices);
    }
    pre_packed_file_input.close();
  }

  logger_->report("[INFO] Total prepacked clusters : {}",
                  pre_packed_vertices_.size());
...
// The function for partitioning a FPGA-based netlist
void TritonPart::PartitionFpgaNetlist(
    ...
    const char* timing_file_arg,
    const char* community_file_arg,
    const char* group_file_arg,
    const char* pre_packed_file_arg)
{
  ...
  std::string timing_file = timing_file_arg;
  std::string pre_packed_file = pre_packed_file_arg;
  // solution file
  std::string solution_file
      = hypergraph_file + std::string(".part.") + std::to_string(num_parts_);
  logger_->setDebugLevel(PAR, "hypergraph", 5);
  logger_->setDebugLevel(PAR, "multilevel_partitioning", 5);
  logger_->setDebugLevel(PAR, "partitioning", 5);
  logger_->setDebugLevel(PAR, "coarsening", 5);
  logger_->setDebugLevel(PAR, "refinement", 5);
  logger_->setDebugLevel(PAR, "evaluation", 5);
  timing_aware_flag_ = timing_file.empty() ? false : true;
  srand(seed_);  // set the random seed
  ReadFpgaGraph(hypergraph_file,
                community_file,
                group_file,
                timing_file,
                pre_packed_file);
                                     ...
                                     const char* placement_file_arg)
{
  ...
  logger_->setDebugLevel(PAR, "hypergraph", 5);
  logger_->setDebugLevel(PAR, "multilevel_partitioning", 5);
  logger_->setDebugLevel(PAR, "initial_partitioning", 5);
  logger_->setDebugLevel(PAR, "partitioning", 5);
  logger_->setDebugLevel(PAR, "coarsening", 5);
  logger_->setDebugLevel(PAR, "refinement", 5);
  logger_->setDebugLevel(PAR, "evaluation", 5);
...
void TritonPart::BuildTimingPaths()
{
  ...
  for (auto& path_end : path_ends) {
    ...
    if (!timing_path.arcs.empty()) {
      // some of the paths are like this:
      // {1, 1, 2, 2, 3, 3}
      // we need to remove the duplicate vertex
      /*std::vector<int> new_path;
      std::vector<int> new_arcs;
      for (size_t i = 0; i < timing_path.path.size(); i++) {
        if (new_path.empty() || new_path.back() != timing_path.path[i]) {
          new_path.push_back(timing_path.path[i]);
        }
      }
      timing_path.path = new_path;*/
      timing_paths_.push_back(timing_path);
    }
  }
...
void TritonPart::MultiLevelPartition()
{
  ...
  // create the FPGrowth class
  int min_support = 10;
  logger_->report("[INFO] Running FP-Growth with min_support = {}",
                  min_support);
  int num_threads = std::thread::hardware_concurrency();
  auto tritonpart_fpgrowth
      = std::make_shared<FPGrowth>(min_support, num_threads);

  // do pattern mining to identigy frequently occuring patterns in the top N
  // timing paths
  // read through timing_paths_ and collect the data as a vector<vector<int>>
  dataset_t timing_paths_fp;
  for (auto& timing_path : timing_paths_) {
    transaction_t transaction;
    for (auto& vertex : timing_path.path) {
      transaction.push_back(vertex);
    }
    timing_paths_fp.push_back(transaction);
  }

  /*dataset_t timing_paths = {
        {1, 2, 3},      // Path 1: Node 1 -> Node 2 -> Node 3
        {2, 3, 4},      // Path 2: Node 2 -> Node 3 -> Node 4
        {1, 3, 4, 5},   // Path 3: Node 1 -> Node 3 -> Node 4 -> Node 5
        {1, 2, 3},      // Path 4: Node 1 -> Node 2 -> Node 3
        {2, 3}          // Path 5: Node 2 -> Node 3
    };*/

  // logger_->report("Total size of dataset : {}", timing_paths_fp.size());

  // start mining now
  //tritonpart_fpgrowth->fit(timing_paths_fp);

  // now print out the most occuring patterns
  // tritonpart_fpgrowth->print_frequent_patterns();
  // auto clusters = tritonpart_fpgrowth->cluster();
  ...
  tritonpart_mlevel_partitioner->SetFPGrowth(true);
  tritonpart_mlevel_partitioner->SetTransactions(timing_paths_fp);
  ...
  // create clusters from pre_packed information
  if (pre_packed_vertices_.empty() == false) {
    std::vector<int> pre_packed_clusters(hypergraph_->GetNumVertices(), -1);
    int cluster_id = 0;
    for (auto& vertices : pre_packed_vertices_) {
      for (auto& vertex : vertices) {
        pre_packed_clusters[vertex] = cluster_id;
      }
      cluster_id++;
    }

    for (int i = 0; i < hypergraph_->GetNumVertices(); i++) {
      if (pre_packed_clusters[i] == -1) {
        pre_packed_clusters[i] = cluster_id;
        cluster_id++;
      }
    }
    // set the pre_packed information
    tritonpart_mlevel_partitioner->SetPrePackedSol(pre_packed_clusters);
    tritonpart_mlevel_partitioner->SetPrePack(true);
  }
