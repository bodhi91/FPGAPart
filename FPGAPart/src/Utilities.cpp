    ...
    const Matrix<float>& lower_block_balance)
{
  ...
  return OptimalPartCplexv2(num_parts,
    ...
    const Matrix<float>& lower_block_balance)
{
  ...
  float max_vertex_weight = 0.0;
  float max_hyperedge_weight = 0.0;

  for (int v = 0; v < num_vertices; v++) {
    float total_vtx_weight = std::accumulate(
        vertex_weights[v].begin(), vertex_weights[v].end(), 0.0);
    max_vertex_weight = std::max(max_vertex_weight, total_vtx_weight);
  }

  for (int e = 0; e < num_hyperedges; e++) {
    max_hyperedge_weight = std::max(max_hyperedge_weight, hyperedge_weights[e]);
  }
  ...
  if (static_cast<int>(solution.size()) == num_vertices) {
    ...
    for (int v = 0; v < num_vertices; v++) {
      if (solution[v] != -1) {  // Ensure valid assignments
        startVarX.add(var_x[solution[v]][v]);
        startValX.add(1);
      }
    }

    // Disable presolve to maintain consistency with the solution
    mycplex.setParam(IloCplex::Param::Preprocessing::Presolve, IloFalse);
    mycplex.extract(mymodel);

    // Add the MIP start
    try {
      mycplex.addMIPStart(startVarX, startValX);
      std::cout << "MIP start added" << std::endl;
  ...
  mycplex.setParam(IloCplex::Param::Preprocessing::Presolve, IloFalse);
  //mycplex.setParam(IloCplex::Param::Parallel, IloCplex::Deterministic); // Deterministic parallelism
  mycplex.setParam(IloCplex::Param::RandomSeed, 42); 
  //mycplex.setParam(IloCplex::HeurFreq, 10);  // Run heuristics every 10 nodes
  //mycplex.setParam(IloCplex::FPHeur, 1);     // Enable feasibility pump
  /*if (static_cast<int>(solution.size()) == num_vertices) {
    IloNumVarArray startVarX(myenv);
    ...
    mycplex.setParam(IloCplex::Param::MIP::Display, 5);
    mycplex.addMIPStart(startVarX, startValX);
  ...
  mycplex.use(new (myenv)
                  NeighborhoodInfluenceCuttingPlanes(myenv,
                                                     var_x,
                                                     var_y,
                                                     hyperedges,
                                                     vertex_weights,
  ...
  /*mycplex.use(new (myenv) HgrLazyConstraintCallback(myenv,
                                                 var_x,
                                                 num_parts,
                                                 num_vertices,
                                                 vertex_weight_dimension,
  ...
  /*mycplex.use(NbrInfluencecallback(myenv,
                                   var_x,
                                   var_y,
                                   hyperedges,
                                   vertex_weights,
  ...
  // Solve the problem
  // mycplex.solve();
  mycplex.exportModel("model.lp");
  mycplex.writeMIPStarts("mipstart.sol");
