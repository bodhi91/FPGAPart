...
sta::define_cmd_args "triton_part_fpga" {
  ...
  [-pre_packed_file pre_packed_file] \
...
proc triton_part_fpga { args } {
  sta::parse_key_args "triton_part_fpga" args \
    keys {-num_parts \
          -balance_constraint \
          -base_balance \
          -scale_factor \
  ...
  if { [info exists keys(-timing_file)] } {
    set timing_file $keys(-timing_file)
  }
  ...
  if { [info exists keys(-pre_packed_file)] } {
    set pre_packed_file $keys(-pre_packed_file)
  }
  ...
  par::triton_part_fpga $num_parts \
    $balance_constraint \
    $base_balance \
    $scale_factor \
    $seed \
