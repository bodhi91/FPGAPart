...
{
 public:
  void SetFPGrowth(bool fp_growth) { fp_growth_ = fp_growth; }
  bool GetFPGrowth() const { return fp_growth_; }
  void SetPrePack(bool pre_pack) { pre_pack_ = pre_pack; }
  bool GetPrePack() const { return pre_pack_; }
  void SetPrePackedSol(const std::vector<int>& pre_packed_solution)
  {
    pre_packed_solution_ = pre_packed_solution;
  }
  void SetTransactions(const std::vector<std::vector<int>>& transactions)
  {
 ...
 private:
  ...
  const HGraphPtr& FPGrowthClustering(const HGraphPtr& hgraph) const;
  ...
  bool fp_growth_ = false;
  bool pre_pack_ = false;
  std::vector<int> pre_packed_solution_;
  std::vector<std::vector<int>> transactions_; // for fp-growth

  // pointers
  CoarseningPtr coarsener_ = nullptr;
  FPGrower fpgrower_ = nullptr;
