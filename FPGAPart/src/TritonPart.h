...
class TritonPart
{
 public:
  ...
  void PartitionFpgaNetlist(unsigned int num_parts,
 ...
 private:
  ...
  void ReadTimingPaths(const std::string& timing_paths_file_name);
  // read and build FPGA (hyper)graph
  void ReadFpgaGraph(const std::string& hypergraph,
  ...
  std::vector<float> hyperedge_criticalities_; 
  std::vector<float> vertex_criticalities_;
  ...
  // molecule_attr_ contains the pre-packed information obtained from VTR
  std::unordered_map<int, int> molecule_attr_;  // only for FPGAs
  std::vector<std::vector<int>> pre_packed_vertices_;
