...
### Set variables ###
num_cuts_values=(2)  # Add your num_cuts values here
...
vtr_designs=("bgm" "blob_merge" "boundtop" "diffeq1" "diffeq2" "LU32PEEng" "mkDelayWorker32B" "mkPktMerge" "mkSMadapter48" "or1200" "raygentop" "sha" "stereovision0" "stereovision1" "stereovision2")
vtr_cwidth=(170 104 94 122 114 352 120 94 128 182 118 76 102 232 320)

koios_designs=("eltwise_layer" "attention_layer" "conv_layer_hls" "reduction_layer" "spmv" "robot_rl" "softmax" "bwave_like.fixed.small" "tpu_like.small.os" "tpu_like.small.ws")
koios_width=(200 224 94 192 214 172 154 200 500 500)
    ...
    for num_cuts in "${num_cuts_values[@]}"; do
        ...
        for i in "${!vtr_designs[@]}"; do
            ...
            for seed in $(seq 0 $((total_seeds - 1))); do
                ...
                echo "mkdir -p ${output_dir}; cd ${output_dir}; rm -rf *; \
                        python3 $PART_RUNNER $VTR_ARCH $VTR_BLIF_LOCATION/${design}.blif \
                        --route_chan_width $current_channel_width --partitioner tritonpart --ub-factor $ub_factor \
                        --graph-model $graph_model --graph-edge-weight $graph_edge_weight --seed $seed $COMMON_OPTS; \
                        mv vpr_stdout.log ${design}_cuts${num_cuts}_pwcut${percent_wires_cut}_seed${seed}.vpr_tritonpart_stdout.log;" >> "${RUN_DIR}/design_vtr_run_cmds"
        ...
        for i in "${!koios_designs[@]}"; do
            ...
            for seed in $(seq 0 $((total_seeds - 1))); do
                ...
                # Generate command and output it to the script file
                echo "mkdir -p ${output_dir}; cd ${output_dir}; rm -rf *; \
                        python3 $PART_RUNNER $VTR_ARCH $KOIOS_BLIF_LOCATION/${design}.abc.blif \
                        --route_chan_width $current_channel_width --partitioner tritonpart --ub-factor $ub_factor \
                        --graph-model $graph_model --graph-edge-weight $graph_edge_weight --seed $seed $COMMON_OPTS; \
                        mv vpr_stdout.log ${design}_cuts${num_cuts}_pwcut${percent_wires_cut}_seed${seed}.vpr_tritonpart_stdout.log;" >> "${RUN_DIR}/design_vtr_run_cmds"
